...
import { data, Link, Navigate, useLocation, useParams } from "react-router-dom";
...
export default function Postdetails() {
    ...
    const relatedPosts = PostsData.filter(p => p.category === post.category && p.id !== post.id).slice(0, 3);

    return (
        <>
            <article className="bg-[#0a0a0a] selection:bg-orange-500 selection:text-white  min-h-screen font-[Tajawal]">
                <section className="relative h-[60vh] min-h-125 overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-linear-to-t from-[#0a0a0a] to-[#0a0a0a34]"></div>
                    <div className="absolute top-8 right-8 flex items-center gap-2 px-4 py-2 bg-black/30 backdrop-blur-md rounded-full text-sm border border-white/10">
                            ...
                            <div className="flex flex-wrap items-center gap-3 mb-6">
                                ...
                                <div className="flex items-center gap-4 text-white/70 text-sm">
                                    <span className="flex items-center gap-2">
                                        <i className="fa-regular fa-calendar"></i> {new Date(post.date).toLocaleDateString("ar-EG", {
                                    year: "numeric",
                                    month: "long",
                                    day: "numeric",
                                })}
                        ...
                        <div className="order-2 lg:order-1">
                            ...
                            <div className="mt-14 p-6 bg-[#111111] rounded-2xl border border-[#262626]">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {post.tags.map((tag) => {
                                        return (
                                            <span key={tag} className="px-4 py-2 bg-[#1a1a1a] text-neutral-400 text-sm rounded-full border border-[#262626] hover:border-orange-500/50 hover:text-orange-500 transition-colors cursor-pointer">
                                                # {tag}
                                            </span>
                                        );
                                    })}
